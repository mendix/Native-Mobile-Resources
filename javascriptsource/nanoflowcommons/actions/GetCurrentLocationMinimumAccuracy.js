 ...
 */
async function GetCurrentLocationMinimumAccuracy(
  timeout,
  maximumAge,
  highAccuracy,
  minimumAccuracy
  ...
  return new Promise((resolve, reject) => {
    ...
    const watchId = geolocationModule.watchPosition(
      onSuccess,
      onError,
      options
    );
    ...
    function createGeolocationObject(position) {
      mx.data.create({
        ...
        error: () =>
          reject(
            new Error(
              "Could not create 'NanoflowCommons.Geolocation' object to store location"
            )
          ),
      });
    }
    function onTimeout() {
      geolocationModule === null || geolocationModule === void 0
        ? void 0
    ...
    function onSuccess(position) {
      if (
        !minimumAccuracy ||
        Number(minimumAccuracy) >= position.coords.accuracy
      ) {
        ...
        if (
          !lastAccruedPosition ||
          position.coords.accuracy < lastAccruedPosition.coords.accuracy
        ) {
    ...
    function mapPositionToMxObject(mxObject, position) {
      ...
      if (
        ...
        mxObject.set(
          "AltitudeAccuracy",
          new Big(position.coords.altitudeAccuracy.toFixed(8))
        );
      }
