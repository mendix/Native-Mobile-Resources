...
async function Geocode(address, geocodingProvider, providerApiKey) {
  ...
  if (navigator && navigator.product === "ReactNative") {
    return Geodecoder.geocodeAddress(address).then((results) => {
      ...
      return createMxObject(
        String(results[0].position.lat),
        String(results[0].position.lng)
      );
    });
  }
  if (!geocodingProvider) {
    return Promise.reject(
      new Error(
        "Input parameter 'Geocoding provider' is required for use on web"
      )
    );
  }
  if (!providerApiKey) {
    return Promise.reject(
      new Error("Input parameter 'Provider api key' is required for use on web")
    );
  }
    ...
    return new Promise((resolve, reject) => {
      mx.data.create({
        ...
        error: () => {
          reject(
            new Error(
              "Could not create 'NanoflowCommons.Position' object to store coordinates"
            )
          );
        },
      });
