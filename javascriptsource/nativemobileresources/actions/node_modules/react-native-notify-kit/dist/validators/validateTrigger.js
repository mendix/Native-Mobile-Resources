 ...
 */
import { objectHasProperty, isNumber, isObject, isValidEnum, isUndefined, isBoolean, isAndroid, } from '../utils';
import { TimeUnit, RepeatFrequency, TriggerType, AlarmType, } from '../types/Trigger';
...
function validateTimestampTrigger(trigger) {
    ...
    if (trigger.timestamp < MIN_PLAUSIBLE_EPOCH_MS) {
        if (trigger.timestamp >= 1 && trigger.timestamp <= 31) {
            throw new Error(`'trigger.timestamp' looks like a day-of-month (${trigger.timestamp}). Did you mean \`date.getTime()\` instead of \`date.getDate()\`?`);
        }
        if (trigger.timestamp >= MIN_PLAUSIBLE_EPOCH_SECONDS) {
            throw new Error(`'trigger.timestamp' looks like seconds since epoch (${trigger.timestamp}). Notifee expects milliseconds — multiply by 1000, or use \`Date.now()\` / \`date.getTime()\`.`);
        }
        throw new Error(`'trigger.timestamp' (${trigger.timestamp}) is too small to be a valid epoch millisecond value. Use \`Date.now()\` or \`someDate.getTime()\`.`);
    }
    const now = Date.now();
    ...
    const hasRepeatFrequency = objectHasProperty(trigger, 'repeatFrequency') && !isUndefined(trigger.repeatFrequency);
    ...
    if (isAndroid &&
        ...
        throw new Error("'trigger.repeatFrequency' MONTHLY is not supported when 'trigger.alarmManager' is false.");
    }
    return out;
}
