 ...
 */
export function reconstructNotification(parsed, remoteMessage, config) {
    const data = rebuildData(remoteMessage.data);
    const title = parsed?.title ?? remoteMessage.notification?.title ?? remoteMessage.data?.title ?? '';
...
function rebuildData(rawData) {
    ...
    if (typeof notifeeDataRaw === 'string') {
        try {
            const notifeeData = JSON.parse(notifeeDataRaw);
...
// ---------------------------------------------------------------------------
function buildAndroidConfig(raw, config) {
    const android = {};
    ...
    if (typeof raw?.smallIcon === 'string')
        android.smallIcon = raw.smallIcon;
    if (typeof raw?.largeIcon === 'string')
        android.largeIcon = raw.largeIcon;
    if (typeof raw?.color === 'string')
        android.color = raw.color;
    // Actions array — pass through (trust server validation)
    if (Array.isArray(raw?.actions)) {
        android.actions = raw.actions;
    }
        ...
        if (typeof style.type === 'string') {
            ...
            if (mappedType !== undefined) {
                if (mappedType === AndroidStyle.BIGTEXT && typeof style.text === 'string') {
                    ...
                    console.warn(`${PREFIX} android.style.type '${style.type}' present but required '${field}' field missing or not a string. Style ignored.`);
                }
            }
            else {
...
function buildIosConfig(raw) {
    ...
    if (typeof raw.interruptionLevel === 'string') {
        if (VALID_INTERRUPTION_LEVELS.has(raw.interruptionLevel)) {
            ios.interruptionLevel = raw.interruptionLevel;
        }
        else {
    ...
    if (Array.isArray(raw.attachments)) {
        ios.attachments = raw.attachments
            .filter((att) => {
...
//# sourceMappingURL=reconstructNotification.js.map